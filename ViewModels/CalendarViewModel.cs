...
public class CalendarViewModel : INotifyPropertyChanged
{
    ...
    private static ICalendarService CreateIcsService(AppSettings settings)
    {
        ...
        if (urls.Count == 1)
        {
            return CreateSingleIcsService(urls[0], GetAlias(aliases, 0, urls[0]),
                SubscriptionColors[0], settings.IcsRefreshMinutes);
        }
        ...
        {
            services.Add(CreateSingleIcsService(urls[i], GetAlias(aliases, i, urls[i]),
                SubscriptionColors[i % SubscriptionColors.Length], settings.IcsRefreshMinutes));
        }
    ...
    /// <summary>
    /// 为单个订阅链接创建服务。URL 含 {year} 占位符时按年份分文件处理
    /// （未来年份可能尚未发布，由 YearlyIcsCalendarService 负责兜底）。
    /// </summary>
    private static ICalendarService CreateSingleIcsService(
        string url, string name, string color, int refreshMinutes)
    {
        if (url.Contains(YearlyIcsCalendarService.YearPlaceholder, StringComparison.OrdinalIgnoreCase))
        {
            return new YearlyIcsCalendarService(url, refreshMinutes, name, color);
        }

        return new IcsCalendarService(url, refreshMinutes, name, color);
    }
    ...
    private void GenerateCalendarGrid(List<CalendarEvent> events)
    {
        ...
        for (int i = 0; i < 42; i++)
        {
            ...
            CalendarDays.Add(new CalendarDay(
                ...
                LunarDate: lunarText,
                Events: dayEvents,
                HolidayKind: DetectHolidayKind(dayEvents)
            ));
        }
    ...
    /// <summary>
    /// 从当天事件中识别放假 / 补班标记。
    /// 依据节假日订阅的标题约定：「元旦(休)」表示放假，「春节(班)」表示调休上班。
    /// 半角括号与全角括号都支持（不同订阅源写法不一）。
    /// 同一天理论上不会同时是休和班，若出现则以「休」优先。
    /// </summary>
    private static HolidayKind DetectHolidayKind(List<CalendarEvent> dayEvents)
    {
        var kind = HolidayKind.None;
        ...
        foreach (var evt in dayEvents)
        {
            ...
            if (title.Contains("(休)", StringComparison.Ordinal) ||
                title.Contains("（休）", StringComparison.Ordinal))
            {
                return HolidayKind.Holiday; // 休优先，直接返回
            }

            if (title.Contains("(班)", StringComparison.Ordinal) ||
                title.Contains("（班）", StringComparison.Ordinal))
            {
                kind = HolidayKind.Workday;
            }
        }
