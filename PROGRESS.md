...
## 总体进度：MVP + 设置界面已完成，ICS 订阅支持按年份分文件 ✅
...
- [x] 界面字体大小（5 档切换）
- [x] 开机自启动开关
- [x] 日历数据源配置（系统日历 / .ics 订阅 / 两者都用，支持多订阅 + 别名）
- [x] 周起始日（周日 / 周一）
- [x] 近期事件显示天数（1/3/7 天）
- [x] .ics 刷新频率（10/30/60/120 分钟）
- [x] 设置持久化（JSON 文件 `%LOCALAPPDATA%/miniCal/settings.json`）
- [ ] 农历显示开关（README 曾列出，实际未实现，农历始终显示）
- [ ] 主界面顶部时间日期显示开关（`ShowHeaderDateTime` 已定义但无界面、无读取方）
- [ ] 时间显示格式（`TimeFormat` 已定义但无界面、无读取方）

### 数据服务
- [x] 日历服务接口（ICalendarService）
- [x] 模拟数据服务（MockCalendarService，用于调试）
- [x] .ics 远程日历服务（IcsCalendarService，使用 Ical.Net 解析）
- [x] 按年份分文件的 .ics 服务（YearlyIcsCalendarService，`{year}` 占位 + 多级兜底）
- [x] 聚合日历服务（AggregateCalendarService，多数据源合并）
- [x] 空日历服务（EmptyCalendarService，无数据源时的默认服务）
- [ ] Windows 日历 API 接入（WindowsCalendarService 已写好但需 Windows SDK，
      在 csproj 中被 `<Compile Remove>` 排除；「系统日历」数据源实际回落到 MockCalendarService）
...
| `Core/Services/YearlyIcsCalendarService.cs` | 按年份分文件的 .ics 服务（⭐ 新增） |
...
>
> ⚠️ 构建需要 **.NET 8 SDK**（仅有运行时无法构建）。本机已安装 8.0.425
> （`winget install Microsoft.DotNet.SDK.8`）。校验：`dotnet --list-sdks` 应列出 8.x。
...
**按年份分文件的 ICS 订阅支持（YearlyIcsCalendarService）**

接入 [chinese-days](https://github.com/vsme/chinese-days) 的中国节假日订阅，该源按年份分文件，
未来年份尚未发布时请求返回 404，因此需要兜底。

**URL 模板**：订阅链接中用 `{year}` 占位，运行时替换为需要的年份：

```
https://cdn.jsdelivr.net/npm/chinese-days/dist/years/{year}.ics
```

`CalendarViewModel.CreateSingleIcsService()` 按 URL 是否含 `{year}` 分流：
含占位符走 `YearlyIcsCalendarService`，否则仍走 `IcsCalendarService`（已有订阅行为不变）。

**兜底链路**（每个年份独立）：

```
内存 → 磁盘缓存 → years/{year}.ics → holidays.ics（聚合兜底）→ 跳过该年份
```

任何一环失败都不抛给调用方，也不清除已有磁盘缓存。

**关键设计**：

1. **区分「缺失」与「网络错误」**。404/410 才算该年份未发布，记 `MissingSince` 进 12 小时冷却，
   避免每次开面板都打一次 404；超时 / DNS 失败 / 5xx 不算缺失，下次照常重试。
2. **软 404 识别**。CDN 对缺失文件有时回 200 + 纯文本，故除状态码外还检查 body 含 `BEGIN:VCALENDAR`。
3. **聚合兜底链接自动推断**：`.../years/{year}.ics` → `.../holidays.ics`；结构不匹配则不启用兜底，不猜 URL。
4. **跨年查询**按 `start.Year..end.Year` 逐年取数据。月历网格前后各留 7 天，1 月和 12 月必然跨年，这条是必要的。
5. **去重只合并完全相同的事件**（StartTime + EndTime + Title）。走聚合兜底时聚合文件也含其他年份数据，
   会与按年份文件重叠；但**刻意不做标题前缀合并**——否则会把同一节日的「休」和「班」误删一条
   （与 `IcsCalendarService` 的前缀去重策略不同，那里是为了处理同名节日的单天/多天重复定义）。
6. 缓存目录与命名规则沿用 `IcsCalendarService`：`%LOCALAPPDATA%/WinCal/cache/{url的SHA256前16位}.ics`，
   按解析后的实际 URL 分文件，聚合文件单独一份供多个缺失年份共用。

**数据源实测**（2026-09-24）：

| 路径 | 状态 |
|------|------|
| `years/2023.ics` ~ `years/2026.ics` | 200，各 12~15 个 VEVENT |
| `years/2027.ics` 及以后 | 404（body 为纯文本说明） |
| `holidays.ics` | 200，41 个 VEVENT，覆盖 2024-01-01 ~ 2026-10-11 |

全部为 `DTSTART;VALUE=DATE` 全天事件，无 RRULE，`DTEND` 均为末日+1（不触发 `EndTime==StartTime` 的旧坑）；
每个按年份文件只含本年事件，无跨年混入。标题形如 `元旦(休)`、`春节(班)`。

**验证方式**：本机原先只有 .NET 6 运行时无 SDK，已安装 .NET 8 SDK（8.0.425）后 `dotnet build` 通过（0 错误 0 警告）。
另用临时控制台项目 + 本地假 HTTP 服务器构造真实 CDN 无法复现的响应组合，验证 12 组场景：
per-year 404 转聚合兜底、404 冷却生效（3 次查询只打 1 次）、5xx 不进冷却仍重试、软 404 识别、
per-year 命中时不混入聚合数据、服务器下线后磁盘缓存仍可用、跨年查询两年数据齐全。临时项目与测试缓存已清理。

> ⚠️ 兜底只能延缓失效，无法造出数据。当前 `years/2027.ics` 未发布且 `holidays.ics` 只到 2026-10，
> 故 2027 年节假日暂时无数据源可用（实测查 2027 全年返回 0 条），需等上游发布。
